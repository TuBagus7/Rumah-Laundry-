...
// Fungsi untuk mengeksekusi query ke database
...
function update_kary($up_kary) {
    ...
    $stmt = mysqli_prepare($koneksi, $up_query);
    if ($stmt === false) {
        die('Prepare failed: ' . htmlspecialchars(mysqli_error($koneksi)));
    }

    mysqli_stmt_bind_param($stmt, 'sssi', $nama, $username, $email, $id_user);
...
function del_kary($id_kary) {
    ...
    $query1 = "DELETE FROM absensi WHERE id_user = '$id_kary'";
    if (!mysqli_query($koneksi, $query1)) {
        error_log("Error deleting from absensi: " . mysqli_error($koneksi));
        return 0;
    }

    // Hapus data di tabel 'master'
    $query2 = "DELETE FROM master WHERE id_user = '$id_kary'";
    if (mysqli_query($koneksi, $query2)) {
        return mysqli_affected_rows($koneksi);
    } else {
        error_log("Error deleting from master: " . mysqli_error($koneksi));
        return 0;
    }
}
...
// Order
function order_ck($order_ck, $file_path) {
    global $koneksi;
    ...
    $insert_ck = "INSERT INTO tb_order_ck (or_ck_number, nama_pel_ck, no_telp_ck, alamat_ck, jenis_paket_ck, wkt_krj_ck, berat_qty_ck, harga_perkilo, tgl_masuk_ck, tgl_keluar_ck, tot_bayar, keterangan_ck, file_path)
                  VALUES ('$orderNum', '$nama_pel', '$no_telp', '$alamat', '$jns_pkt', '$wkt_kerja_ck', '$berat_qty', '$tarif_perkilo', '$tgl_masuk', '$tgl_keluar', '$total_bayar', '$ket', '$file_path')";
...
function order_dc($order_dc, $file_path) {
	global $koneksi;
	...
	$query_dc = "INSERT INTO tb_order_dc VALUES (
		...
		'$berat_dc','$trf_dc','$tgl_msk_dc','$tgl_kel_dc','$tot_bayar_dc','$ket_dc','	$file_path'
	)";
...
function order_cs($order_cs,$file_path){
	global $koneksi;
	...
	$query_cs = "INSERT INTO tb_order_cs VALUES (
		...
		'$wkt_krj_cs','$jml_pcs','$trf_cs','$tgl_msk_cs','$tgl_kel_cs','$totBayar_cs','$ket_cs','$file_path'
	)";
	mysqli_query($koneksi,$query_cs);
	return mysqli_affected_rows($koneksi);
}
