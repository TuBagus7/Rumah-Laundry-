...
if (isset($_POST['order_dc'])) {
    $uploadDir = 'C:/xampp/htdocs/rumah_laundry/uploads/file_uploads_dc/'; // Direktori tempat file diunggah
    $uploadFile = $uploadDir . basename($_FILES['file_upload']['name']);
    $file_path = null;

    if (move_uploaded_file($_FILES['file_upload']['tmp_name'], $uploadFile)) {
        $file_path = 'uploads/file_uploads_dc/' . basename($_FILES['file_upload']['name']); // Simpan jalur relatif
    }

    if (order_dc($_POST, $file_path) > 0) {
